...
// Search and Filter Tools by Text Input
function searchTools() {
    const input = document.getElementById("search").value.toLowerCase();
    const tools = Array.from(document.querySelectorAll(".tool"));
    const container = document.getElementById("toolContainer");

    // Filter tools based on input match
    const filteredTools = tools.filter(tool =>
        tool.innerText.toLowerCase().includes(input)
    );

    // Clear and re-populate tool list with matched tools first
    container.innerHTML = "";
    filteredTools.forEach(tool => container.appendChild(tool));

    // Optionally show unmatched tools below (remove if not needed)
    tools.filter(tool => !filteredTools.includes(tool))
         .forEach(tool => container.appendChild(tool));
}
